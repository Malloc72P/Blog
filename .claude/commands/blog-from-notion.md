...
2. **AI가 아닌 글쓴이가 쓴 것처럼**: 기존 포스트의 문체를 기준점으로, 담백한 존댓말을 유지하고 AI 티(상투적 도입/마무리, 공허한 수식어, 기계적 불릿, 번역체)를 제거한다. 노션 노트에 있는 사실만 쓰고, 작성자가 겪지 않은 1인칭 일화를 지어내지 않는다. 비유·반전·여운 문장 없이 현상과 규칙을 그대로 쓴다.
...
- **문체 가이드 추출**: Explore 에이전트로 기존 포스트 여러 편(예: `frontend/closure`, `frontend/hoisting`, 이미지 사례 `ai/git-worktree`·`ai/sse-exam`)을 정독해 어조·구조·이미지 삽입 문법·`@libs/frontmatter` API를 정리한다.
