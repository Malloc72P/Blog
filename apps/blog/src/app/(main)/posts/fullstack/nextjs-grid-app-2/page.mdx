...
export const metadata = frontmatter({
  title: 'Prisma ORM으로 DB를 연결하고 서버 액션에서 CRUD 구현하기 (2편)',
  description:
    'Prisma를 설치해 스키마를 정의하고 개발용 DB를 연결한 뒤, PrismaClient 인스턴스를 관리하며 생성·조회·수정·삭제 서버 액션을 구현합니다.',
  seriesId: 'fullstack',
...
[지난 포스트](/posts/fullstack/nextjs-grid-app-1)에선 Next.js에 대한 간단한 소개와 클라이언트에서 서버의 비즈니스 로직을 호출하는 방법에 대해 배웠습니다.
...
- 실행하면 http://localhost:5555/ 주소로 웹페이지가 하나 열릴텐데, 에러 메시지가 표시되지 않는다면, 정상적으로 연결된 것을 의미합니다.
...
[다음 포스트](/posts/fullstack/nextjs-grid-app-3)에선 AG Grid를 사용해서 화면을 구현해보려고 합니다.  
긴 글 읽어주셔서 감사합니다.
