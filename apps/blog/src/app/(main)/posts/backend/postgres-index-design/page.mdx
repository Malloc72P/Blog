...
export const metadata = frontmatter({
  title: 'PostgreSQL DESC 인덱스와 커버링 인덱스 성능',
  description:
...
이 글은 그 측정 기록입니다. B-tree 인덱스가 방향에 따라 어떻게 동작하는지부터 보고, 인덱스를 뒤에서부터 읽으면 정말 느려지는지, 조회할 컬럼 값을 인덱스에 같이 저장해 두면 언제 이득인지를 차례로 확인합니다.
...
이 주장대로 `Index Scan Backward`, 즉 역방향 스캔이 잡혔습니다. 여기까지는 주장과 일치합니다.
...
정방향 순차 읽기는 OS나 스토리지의 **미리 읽기(read-ahead)** 혜택을 받지만 역방향은 그 혜택을 받기 어렵습니다.
...
다만 그렇더라도 인덱스를 하나 더 만들 만한 이유는 되지 않습니다.
...
각 `service` 그룹을 전부 읽어서 그 안에서 다시 정렬해야 하므로 `LIMIT`이 적용되지 않습니다.
...
단일 컬럼일 때는 방향이 무의미했지만, 복합 인덱스에서는 방향이 결정적입니다.
...
1000행을 돌려주려고 힙을 1999번 찾아갔습니다. `Index Only Scan`이라는 이름과 달리 힙을 반복해서 읽었습니다.
...
이 주장이 그럴듯하게 들리는 이유는 말이 되기 때문입니다. 역순으로 읽으면 뭔가 손해일 것 같고, 방향을 맞춰 주면 이득일 것 같습니다. 하지만 B-tree 리프 페이지는 양방향으로 연결돼 있어서 이 직관은 맞지 않습니다. 이런 주장이 맞는지는 `EXPLAIN ANALYZE`로 직접 확인해야 합니다.

이 글에서 확인한 내용은 세 가지로 요약됩니다. 단일 컬럼 인덱스는 뒤집어 읽을 수 있어 방향이 성능에 영향을 주지 않지만, NULLS 위치까지 지정하면 예외입니다. 정렬 방향이 섞이는 복합 인덱스는 방향을 맞춰야 합니다. 커버링 인덱스는 조회할 컬럼 값을 함께 저장하는 만큼 인덱스가 커지므로, 읽는 행이 많을 때만 이득입니다.
