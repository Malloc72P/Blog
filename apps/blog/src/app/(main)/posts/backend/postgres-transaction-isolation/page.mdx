...
export const metadata = frontmatter({
  ...
  description:
    'psql 세션 두 개로 PostgreSQL 18의 격리 수준 네 가지에서 트랜잭션을 직접 실행해 Dirty Read·Non-repeatable Read·Phantom Read·Write Skew가 발생하는 조건을 확인합니다. READ UNCOMMITTED가 없는 이유와 SERIALIZABLE의 검사 규칙을 정리합니다.',
  seriesId: 'backend',
...
SERIALIZABLE은 REPEATABLE READ와 똑같이 동작하고, 직렬 실행과 다른 결과가 나올 수 있는 조건을 추가로 검사합니다. 검사는 세 단계이고, 출처는 문서 13.2.3절과 소스 트리의 [README-SSI](https://github.com/postgres/postgres/blob/master/src/backend/storage/lmgr/README-SSI)입니다.

### 1단계: 읽은 대상에 남기는 표시(SIREAD lock)

SERIALIZABLE 트랜잭션이 읽은 행(또는 행이 든 page, 테이블 전체)에 `SIREAD` lock을 겁니다. 이 글에서는 이 lock을 표시라고 부릅니다. 이름에 lock이 붙어 있지만 이 lock은 다른 트랜잭션을 기다리게 하지 않습니다.
...
간선은 순서가 반대일 때도 생깁니다. 다른 트랜잭션이 이미 고친 행을 SERIALIZABLE 트랜잭션이 읽으면, 읽는 쪽은 자기 스냅샷에 맞는 옛 버전을 읽습니다. 이때도 PostgreSQL은 읽은 트랜잭션에서 쓴 트랜잭션 방향으로 간선을 기록합니다. 소스에서는 `CheckForSerializableConflictOut`이 이 검사를 합니다.

> These "rw-conflicts" occur when one transaction attempts to read data which is not visible to it because the transaction which wrote it (or will later write it) is concurrent.
...
세션 2가 커밋할 때 세션 1은 취소되지 않았습니다. 세션 1은 그 뒤에도 SELECT를 두 번 더 정상 실행했습니다. 이 실험에서 취소는 세션 1이 직접 쓰기를 시도해 두 번째 간선이 생길 때 일어났습니다.

취소가 항상 쓰기나 커밋에서 일어나는 것은 아닙니다. 2단계에서 봤듯 읽기도 간선을 만들기 때문에, 위험 구조가 읽기에서 드러나면 SELECT도 직렬화 실패로 끝날 수 있습니다. `CheckForSerializableConflictOut`에는 읽는 도중의 취소 사유(`Canceled on conflict out to old pivot` 등)가 정의돼 있습니다. 그래서 재시도는 쓰기 문장이나 COMMIT 단위가 아니라 트랜잭션 전체 단위로 해야 합니다.
...
- SERIALIZABLE은 REPEATABLE READ에 검사 단계를 더한 것입니다. 읽은 자리에 SIREAD 표시를 남기고, 남이 그 위에 쓰거나 남이 고친 행을 읽으면 rw-conflict 간선을 기록하고, 한 트랜잭션에 들어오는 간선과 나가는 간선이 둘 다 생기면 취소합니다. 취소는 읽기, 쓰기, 커밋 어디에서든 일어날 수 있어 트랜잭션 전체를 재시도하는 코드가 필요합니다.
