...
export const metadata = frontmatter({
  ...
  description:
    'psql 세션 두 개로 네 격리 수준을 직접 돌려 Dirty Read·Non-repeatable Read·Phantom Read·Write Skew가 어디서 생기고 어디서 막히는지 확인했습니다. READ UNCOMMITTED가 없는 이유, REPEATABLE READ가 팬텀을 막는 이유, SERIALIZABLE이 실제로 검사하는 규칙을 PostgreSQL 18 문서와 실측으로 정리합니다.',
  seriesId: 'backend',
...
REPEATABLE READ와 똑같이 동작하되, 감시자가 하나 더 붙습니다. 감시자가 하는 일은 세 단계이고, 출처는 문서 13.2.3절과 소스 트리의 [README-SSI](https://github.com/postgres/postgres/blob/master/src/backend/storage/lmgr/README-SSI)입니다.
