...
export const metadata = frontmatter({
  ...
  description:
    '데코레이터가 클래스와 메서드에 어떻게 끼어드는지 직접 구현하며 확인하고, NestJS에서 보던 Logger와 Validator를 같은 원리로 만들어 봅니다.',
  seriesId: 'frontend',
  postId: 'ts-decorator',
...
참고로 데코레이터 관련 옵션에는 `experimentalDecorators` 외에 `emitDecoratorMetadata`도 있습니다.
전자는 데코레이터 문법 자체를 활성화하는 옵션이고, 후자는 타입 정보를 메타데이터로 자동 방출해 주는 옵션입니다.
이 글의 Validator 예제는 `reflect-metadata`의 API를 직접 호출해 메타데이터를 저장하므로 `emitDecoratorMetadata`까지 켤 필요는 없고, `experimentalDecorators`만 있으면 충분합니다.
...
console.log(returnValue); // Name: Ina, Age: 20
...
본 포스트에서 다룬 코드는 [데코레이터 예제 저장소](https://github.com/Malloc72P/Blog/tree/main/playground/decorator)에서 확인할 수 있습니다.
