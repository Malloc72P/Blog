...
export const metadata = frontmatter({
  title: '디바운싱과 쓰로틀링',
  description:
    '디바운싱과 쓰로틀링을 각각 언제 써야 하는지 구분하고, 자동 검색 입력과 pointermove 이벤트를 예로 두 방식을 직접 구현해 봅니다.',
  seriesId: 'frontend',
  postId: 'debouncing-and-throttling',
...
디바운스란 이벤트 발생 후 일정 시간동안 해당 이벤트가 재발생하지 않으면 콜백을 호출하는 것을 말합니다.

### 언제 사용하나요?

자동검색기능을 구현하는 경우, Input에서 keyup이 발생할 때 마다 검색 요청을 하는 콜백을 호출하면 너무 빈번하게 요청하게 되어 문제가 됩니다.
이 때, keyup이 발생한 뒤 일정 시간동안 이벤트가 재발생하지 않았다면, 사용자가 해당 키워드로 검색을 원하는 상황으로 간주하고
...
이후, 일정시간이 지나기 전 keyup 이벤트가 재발생하면 클로져 변수에 담긴 setTimeout id를 통해
...
function main() {
  ...
  // 클로져에서 참조하는 변수. 이전 콜백이 호출된 시간을 기록한다.
  let lastTime = 0;

  box.addEventListener('pointermove', (e) => {
    const { x, y } = e;

    // 이전 호출 시간으로부터 일정시간(interval)이 지났는지 검사한다.
    const now = Date.now();
    if (now - lastTime < interval) {
      // 아직 일정시간이 지나지 않았다면 콜백을 호출하지 않는다.
      return;
    }

    // 일정시간이 지났다면 마지막 호출 시간을 갱신하고 콜백을 실행한다.
    lastTime = now;
    counter++;
