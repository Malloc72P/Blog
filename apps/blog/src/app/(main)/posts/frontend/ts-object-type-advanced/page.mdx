...
// 숫자 인덱서 자리에는 Pet, 문자열 인덱서 자리에는 Animal이 들어갑니다.
const pet1: Pet = { name: 'pet1', owner: 'me' };
const pet2: Pet = { name: 'pet2', owner: 'you' };
...
참고로 위 예제에서는 숫자 키의 값을 객체 리터럴로 직접 넣지 않고, `Pet` 타입으로 선언한 변수를 거쳐 할당했습니다. 객체 리터럴을 곧바로 넣으면 뒤에서 살펴볼 초과 속성 검사가 동작해, `owner`가 문자열 인덱서의 값 타입인 `Animal`에 없는 속성으로 잡히기 때문입니다.
...
인덱스 시그니쳐는 다른 속성에도 영향을 미칩니다. 인덱스 시그니쳐가 선언되어 있으면, 명시적으로 선언한 모든 명명 속성(`1`, `length` 등)은 해당 객체에 적용되는 인덱스 시그니쳐의 값 타입과 호환되어야 합니다. 첫 번째 예제에서는 `1`이 숫자 인덱서의 값 타입인 `string`과 호환되지 않아, 두 번째 예제에서는 `1`과 `length`가 문자열 인덱서의 값 타입인 `string`과 호환되지 않아 타입 오류가 발생합니다.
...
name, color 둘 다 옵셔널 속성이므로, 해당 속성이 없어도 컴파일 에러가 발생하지 않습니다. 하지만 객체 리터럴을 직접 전달하는 경우, `colour`라는 없는 속성이 있다는 이유로 에러가 발생합니다.

한편, 똑같은 속성 값을 가지는 객체를 변수에 먼저 할당한 후 전달하면 에러가 발생하지 않습니다.

## 초과 속성 검사를 하는 경우

이렇게 동작하는 이유는, 대상 타입이 명시된 자리에 객체 리터럴이 직접 올 때(변수 할당 또는 함수 인자 전달)만 초과 속성 검사를 하기 때문입니다.

타입스크립트 핸드북에 따르면, 타입스크립트는 객체 리터럴을 (선언된 타입을 가진) 변수에 할당하거나 함수 인자로 전달할 때, 대상 타입에 없는 초과 속성을 가지고 있으면 버그일 수도 있다고 판단한다고 합니다.

`literalExample`에 객체 리터럴을 직접 넘긴 경우는 함수 인자 전달에 해당하므로 검사가 동작합니다. 반면 `castleBravo`는 타입을 명시하지 않아 추론된 타입에 `colour`가 그대로 포함되고, 이후 `Rocket` 파라미터로 전달될 때는 구조적 호환성만 확인하므로 통과합니다.

| 상황                              | 초과 속성 검사 |
| --------------------------------- | -------------- |
| 객체 리터럴을 직접 할당/인자 전달 | 검사함         |
...
| 초과 속성 검사       | 대상 타입이 명시된 자리에 객체 리터럴이 직접 올 때 발생 |
...
- 초과 속성 검사는 대상 타입이 명시된 자리에 객체 리터럴이 직접 올 때(변수 할당 또는 함수 인자 전달) 발생하며, 이는 타이핑 실수로 인한 버그를 방지하기 위한 것이다.

긴 글 읽어주셔서 감사합니다. [다음 포스트](/posts/frontend/ts-type-vs-interface)에서는 type과 interface의 차이점과 Mapped Type에 대해 알아보겠습니다.
