...
export const metadata = frontmatter({
  ...
  description:
    'TypeScript enum이 남기는 런타임 코드와 주의할 점을 설명하고, as const 객체와 (typeof X)[keyof typeof X] 패턴으로 값은 enum처럼 쓰면서 타입은 문자열 리터럴 유니온으로 좁히는 방법을 코드로 정리합니다.',
  seriesId: 'frontend',
  postId: 'ts-as-const',
...
상태 값이나 옵션처럼 정해진 값 몇 개를 코드 곳곳에서 쓸 때 `enum`을 쓰면 값을 한곳에 모아두고 이름으로 참조할 수 있습니다. 다만 `enum`은 타입 정보로 끝나지 않고 **런타임에 실제 코드를 남깁니다**.

이 글에서는 `enum`이 남기는 코드와 주의할 점을 먼저 보고, `as const` 객체와 `(typeof X)[keyof typeof X]` 패턴으로 같은 목적을 더 가볍게 달성하는 방법을 정리합니다.
...
타입 하나 정의했을 뿐인데 즉시 실행 함수(IIFE)가 객체를 만들어냅니다. 게다가 자세히 보면 키와 값을 **양방향으로** 넣고 있습니다. `Direction['Up']`은 `0`이고, `Direction[0]`은 `'Up'`입니다. 숫자 enum이 만드는 역방향 매핑(reverse mapping)입니다. 요청하지 않은 코드가 번들에 그대로 포함됩니다.

이 역방향 매핑은 문제를 일으키기도 합니다. `Object.keys`로 키를 순회하면 값이 두 배로 나옵니다.
...
런타임 코드를 피하고 싶다면 `const enum`이 있습니다. `const enum`은 객체를 만들지 않고, 사용하는 자리에 값을 직접 인라인합니다.
...
값을 인라인하려면 컴파일러가 enum 선언을 볼 수 있어야 합니다. 문제는 컴파일된 라이브러리가 넘겨주는 ambient const enum(`.d.ts`의 `declare const enum`)입니다. 파일을 하나씩 독립적으로 변환하는 환경에서는 그 값을 알 수 없어, `isolatedModules`가 켜져 있으면 아예 에러로 막습니다.
...
정리하면, 일반 `enum`은 원치 않는 런타임 코드와 nominal 타입의 경직성이 문제고, `const enum`은 환경 제약이 문제입니다. 값과 타입을 분리해서 직접 조합하는 방법이 대안이 됩니다.

## as const로 리터럴 타입 고정하기

여기서 쓰는 도구가 `as const`입니다. 평범한 객체는 그 프로퍼티 타입이 넓게 추론됩니다.
...
값이 `'UP'`인데도 타입은 `string`으로 넓어집니다. 나중에 `direction.Up`을 다른 문자열로 바꿔도 괜찮다고 보기 때문입니다. 여기에 `as const`를 붙이면 결과가 달라집니다.
...
값은 준비됐으니 이제 타입이 필요합니다. `enum`은 이름 하나가 값과 타입 역할을 겸했지만, `as const` 객체에서는 타입을 따로 만듭니다. 여기서 `typeof`와 `keyof`를 이어 붙입니다.
...
`as const`에 `satisfies`를 더하면 값은 좁게 유지하면서 형태까지 검사할 수 있습니다. 예를 들어 모든 값이 문자열이어야 한다는 제약을 걸고 싶다면 이렇게 씁니다.
...
`satisfies Record<string, string>`는 각 값이 문자열인지 검사하되, 타입을 `Record<string, string>`으로 넓혀버리지 않습니다. `as const` 덕분에 `Up`의 타입은 여전히 `'UP'` 리터럴로 남습니다. 실수로 값에 숫자를 넣으면 위처럼 그 자리에서 잡습니다.
...
`as const`와 `keyof typeof`는 각각 보면 사소한 문법이지만, 이어 붙이면 `enum`을 대체하는 실전 패턴이 됩니다. 번들에 원치 않는 코드가 포함되는 문제와 역방향 매핑 때문에 순회 결과가 꼬이는 문제를 모두 피할 수 있습니다. `enum` 대신 `as const` 객체와 `(typeof X)[keyof typeof X]` 패턴을 함께 쓰는 것도 방법입니다. 긴 글 읽어주셔서 감사합니다.
