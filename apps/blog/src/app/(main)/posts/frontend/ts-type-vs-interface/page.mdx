...
`&` 연산자는 두 타입 간의 교집합을 만들기 때문에, 두 유니온의 공통 멤버(값)인 `HEAVY`와 `LIGHT`만 지정됩니다.
...
만약 어떤 타입을 새로 만들려는데, 이것이 어떤 기반 타입으로부터 파생되는 상황이라면, 리스코프 치환 원칙을 컴파일 타임에 타입 검사로 지킬 수 있는 `interface-extends` 방법이 좋습니다. 즉, 계층적 타입을 만들 때는 extends를 사용하는 것이 좋습니다.
