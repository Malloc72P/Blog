...
함수 코드는 한 글자도 바뀌지 않았는데 `this`가 가리키는 대상이 달라졌고, 그 탓에 `this.name`을 읽다 실패한 것입니다. 이 차이를 만드는 건 함수의 내용이 아니라 **함수를 어떻게 호출했는가**입니다. 이 글에서는 호출 방식별로 `this`가 정해지는 규칙을 정리합니다.

## 호출 시점에 정해지는 this

`this`를 어렵게 느끼는 이유는 대부분 한 가지 오해에서 출발합니다. 함수를 정의할 때 `this`가 정해진다고 생각하는 것입니다. 하지만 화살표 함수를 제외하면, `this`는 함수가 **호출되는 시점**에 결정됩니다. 같은 함수라도 호출 방식이 다르면 `this`도 달라집니다.

그래서 `this`를 읽을 때는 **호출하는 코드의 형태**를 봐야 합니다. 호출 형태는 크게 다섯 가지로 나눌 수 있고, 각각 `this`를 정하는 규칙이 다릅니다. 하나씩 보겠습니다.
...
엄격 모드(`'use strict'`)에서 독립 호출의 `this`는 `undefined`입니다.

비엄격 모드에서는 `this`가 전역 객체(브라우저에서는 `window`, Node.js에서는 `globalThis`)가 됩니다.
...
`this`는 "이 메서드가 어느 객체 안에 정의됐는가"가 아니라 "호출 시점에 점 앞에 무엇이 있었는가"로 정해집니다. 도입부에서 본 문제도 같은 원리였습니다.
...
`const greet = person.greet`는 함수만 꺼내 온 것이고, `greet()`는 점이 없는 독립 호출입니다. 점 앞에 객체가 없으니 규칙 ①이 적용되어 `this`가 `undefined`가 됩니다. 메서드를 변수에 담거나 콜백으로 넘길 때 `this`가 끊기는 현상은 거의 모두 이 한 가지 이유에서 출발합니다.
...
`this`를 호출 방식에 맡기지 않고 직접 지정하는 방법입니다.
...
`bind`로 고정한 `this`는 나중에 `call`이나 `apply`로 바꾸려 해도 바뀌지 않습니다.
...
앞의 네 규칙은 모두 "호출 방식이 `this`를 정한다"는 원칙을 따랐습니다. 화살표 함수는 이 원칙을 따르지 않습니다.

화살표 함수에는 **자신만의 `this` 바인딩**이 없습니다. 호출 방식과 무관하게, 함수가 정의된 위치의 상위 스코프에 있는 `this`를 그대로 빌려 씁니다. 이것을 렉시컬(lexical) `this`라고 부릅니다. 여기서 렉시컬은 "코드가 작성된 위치"를 뜻합니다. 호출 시점이 아니라 작성된 자리가 기준입니다.
...
화살표 함수의 `this`는 정의된 위치에서 결정되므로, `call`이나 `apply`로도 바꿀 수 없습니다.
...
## setInterval과 이벤트 콜백에서 this가 깨지는 이유
...
class Timer {
  ...
  start() {
    setInterval(function () {
      // setInterval은 이 콜백을 Timer의 메서드로 부르지 않는다
      this.seconds += 1; // this가 Timer 인스턴스가 아니어서 NaN이 된다
...
`start`는 메서드 호출이라 그 안의 `this`는 `Timer` 인스턴스가 맞습니다. 문제는 `setInterval`에 넘긴 **콜백 함수**입니다. `setInterval`은 이 콜백을 `Timer` 인스턴스의 메서드로 호출하지 않고, `this`를 따로 정해서 호출합니다. 브라우저는 `this`를 전역 객체(`window`)로, Node.js는 타이머 객체(`Timeout`)로 정합니다. 그래서 콜백 안의 `this.seconds`는 `Timer` 인스턴스의 값이 아닌 `undefined`이고, `undefined + 1`의 결과인 `NaN`이 찍힙니다. 에러가 나지 않아서 알아차리기가 더 어렵습니다.

콜백으로 함수를 넘기면, 그 함수가 어디서 정의됐는지는 중요하지 않습니다. **누가 어떻게 호출하느냐**가 `this`를 정하기 때문입니다.
...
- 객체의 메서드는 **메서드 축약형**으로 정의합니다. 메서드는 `this`로 자기 객체를 가리켜야 하는데, 화살표 함수로 정의하면 상위 스코프의 `this`를 빌려 와 객체를 못 가리키게 됩니다.
...
객체 리터럴의 프로퍼티 자리에 화살표 함수를 바로 쓰면 더 분명하게 어긋납니다. 그 화살표 함수의 상위 스코프는 객체를 둘러싼 바깥 스코프이기 때문입니다.
...
- 콜백 안에서 바깥의 `this`를 그대로 쓰고 싶다면 **화살표 함수**를 씁니다. `setInterval`, 이벤트 핸들러, 배열 메서드 콜백처럼 직접 호출하지 않고 다른 코드가 대신 호출할 때, 화살표 함수가 바깥 `this`를 안전하게 이어 줍니다.

- `this`를 명시적으로 한 번 정해 고정하고 싶다면 `bind`를 씁니다. 콜백을 여러 곳에 넘겨야 하거나, 화살표 함수로 감싸기 곤란한 상황에서 유용합니다.

- 다른 객체의 메서드를 빌려 한 번만 실행하고 싶다면 `call`이나 `apply`를 씁니다. 유사 배열 객체에 배열 메서드를 적용하는 경우가 대표적입니다.
...
- `this`는 함수를 **호출할 때** 결정된다. 화살표 함수만 정의 위치 기준으로 정해진다는 점에서 예외다.
...
- 콜백에서 `this`가 깨지는 버그는 대부분 콜백이 메서드 호출 형태로 불리지 않아서 생기며, 화살표 함수나 `bind`로 해결한다.

## 마치며

`this`를 읽을 때는 함수 정의보다 호출 형태를 먼저 봅니다. 이 글의 다섯 가지 규칙 중 어느 것이 적용되는지 확인하면 `this` 관련 버그 대부분의 원인을 찾을 수 있습니다. 긴 글 읽어주셔서 감사합니다.
