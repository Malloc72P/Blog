...
화살표 함수는 **자신만의 `this`**를 갖지 않습니다. 호출 방식과 무관하게, 함수가 정의된 위치의 상위 스코프에 있는 `this`를 그대로 빌려 씁니다. 이것을 렉시컬(lexical) `this`라고 부릅니다. 여기서 렉시컬은 "코드가 작성된 위치"를 뜻합니다. 호출 시점이 아니라 작성된 자리가 기준입니다.
...
`start`는 메서드 호출이라 그 안의 `this`는 `Timer` 인스턴스가 맞습니다. 문제는 `setInterval`에 넘긴 **콜백 함수**입니다. 이 콜백은 나중에 `setInterval`이 내부에서 점 없이 독립 호출하므로 규칙 ①이 적용됩니다. 콜백 안의 `this`는 `Timer` 인스턴스와 상관없어집니다.
