...
- **경험은 실제로 있었던 것만 쓴다**: 작성자가 겪지 않은 일화("저도 처음엔 ~했습니다")를 지어내지 않는다. AI가 쓰는 초안에는 1인칭 일화를 넣지 않고, 작성자가 알려준 실제 경험만 쓴다.
- **메커니즘 중심 설명**: 개념 정의에 그치지 않고, 코드 예시나 설정 값을 통해 실제 작동 방식을 상세히 다룬다.
- **비유 없이 직접 설명**: 비유·의인화·반전·여운 문장 없이 현상과 규칙을 그대로 쓴다. 비유 하나를 글 전체에 반복하지 않는다.
- **제목은 주제를 부르는 명사구**: 부제, "완전 정복", "제대로", 질문형을 쓰지 않는다. 검색어가 되는 핵심 용어는 제목에 남긴다.
- **과도한 수식어 지양**: "최고의", "압도적인", "혁명적인" 같은 근거 없는 과장 표현을 피한다. 인사말이나 마무리 표현은 이 규칙의 적용 대상이 아니다.
